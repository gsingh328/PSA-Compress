...
class CrossChannelSublayer(nn.Module):
    def __init__(self, in_patches, in_channels, layerscale_init=0.1, hidden_factor=4):
        ...
        self.layers = nn.Sequential(
            dsnn.DeepBSpline('fc', in_channels, **opt_params),
            nn.Linear(in_channels, h),
            dsnn.DeepBSpline('fc', h, **opt_params),
    ...
    def forward(self, x):
        ...
        x_shape = x.shape
        x = x.reshape(x.shape[0] * x.shape[1], x.shape[2]).contiguous()
        x = self.layers(x)
        x = x.reshape(x_shape).contiguous()
