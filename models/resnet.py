...
class Cifar10Shortcut(nn.Module):
    def __init__(self, planes):
        super(Cifar10Shortcut, self).__init__()
        self.planes = planes
    ...
    def __init__(self, in_planes, planes, stride=1, option='A', expansion=1):
        ...
        if stride != 1 or in_planes != planes:
            if option == 'A':
                ...
                """
                self.shortcut = Cifar10Shortcut(planes)
            elif option == 'B':
                self.shortcut = nn.Sequential(
...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, expansion=1, **kwargs):
