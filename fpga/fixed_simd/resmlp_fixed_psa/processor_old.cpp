...
void dsp_int8_packed_mul(
	...
	#pragma HLS bind_op variable=dsp_O impl=dsp
...
void processor(
	...
	// TILE_SIZE number of elements will index into this so partition completely
	// Make sure to use LUTRAM due to high fanout
	static model_psa_t psa_lut[LERP_PSA_N];
//	#pragma HLS bind_storage variable=psa_lut type=RAM_1WNR impl=bram
//	#pragma HLS array_reshape variable=psa_lut type=complete
	#pragma HLS array_partition variable=psa_lut type=complete

	// Buffers for storing PSA output
	static accel_io_t x_psa_vec[TILE_SIZE];
	#pragma HLS array_partition variable=x_psa_vec type=complete
...
vec_lp:
	...
	#pragma HLS PIPELINE II=1
		...
		// -----------------------------------------------------------------
		// Read a vector of PSA LUT
		// -----------------------------------------------------------------
		for (dim_t itile = 0; itile < LERP_PSA_N / TILE_SIZE; itile++) {
		#pragma HLS UNROLL

			const dim_t idx = (input_ifeat * (LERP_PSA_N / TILE_SIZE)) + itile;

			for (dim_t i = 0; i < TILE_SIZE; i++) {
			#pragma HLS UNROLL

				psa_lut[(itile * TILE_SIZE) + i] = psa_lut_buff[idx][i];
			}
		}
		...
		// Index into PSA LUT
		// -----------------------------------------------------------------
		lerp_psa_lookup(x_aff_vec, apply_psa, psa_lut, x_psa_vec);
		...
		for (dim_t i = 0; i < TILE_SIZE; i++) {
			...
			if (transpose_input) {
				x_tile[xw_wr_idx][i][i_vec % TILE_SIZE] = x_psa_vec[i];
			} else {
				x_tile[xw_wr_idx][i_vec % TILE_SIZE][i] = x_psa_vec[i];
			}
		}
