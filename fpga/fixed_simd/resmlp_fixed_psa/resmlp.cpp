...
#define RESMLP_AXI_PSA_LUT_INPUTS \
	psa_lut_vec
#define RESMLP_AXI_PARAM_INPUTS \
	RESMLP_AXI_LIN_WBS_INPUTS, RESMLP_AXI_AFF_WBS_INPUTS, RESMLP_AXI_PSA_LUT_INPUTS
#define RESMLP_BUFF_INPUTS\
	io_buff, res_buff, w_buff, b_buff, s_buff, w_aff_buff, b_aff_buff, s_aff_buff, psa_lut_buff
#define RESMLP_BUFF_INDICES(IO_X_IDX, IO_Y_IDX, LIN_WBS_IDX, AFF_WBS_BASE_IDX)\
	IO_X_IDX, IO_Y_IDX, LIN_WBS_IDX, AFF_WBS_BASE_IDX
#define RESMLP_LAYER_PARAM_OFFSETS(N)\
	RM_LAYER_##N##_FC_W_OFFSET, RM_LAYER_##N##_FC_BS_OFFSET, RM_LAYER_##N##_AFF_W_OFFSET, RM_LAYER_##N##_AFF_BS_OFFSET, RM_LAYER_##N##_PSA_LUT_OFFSET

// Alternate b/w these two to ensure overlap
// b/w processor and ld_processor_buff_task

// io 0 -> 1, linear params in 0, affine params in (0,0+1,0+2)
#define RESMLP_LAYER_INDICES_CONFIG_1 RESMLP_BUFF_INDICES(0, 1, 0, 0)
// io 1 -> 0, linear params in 1, affine params in (3,3+1,3+2)
#define RESMLP_LAYER_INDICES_CONFIG_2 RESMLP_BUFF_INDICES(1, 0, 1, 3)
// -----------------------------------------------------------------------------
...
void krnl_resmlp(
	...
	#pragma HLS INTERFACE m_axi bundle=gmem0 port=psa_lut_vec max_widen_bitwidth=PORT_VEC_SIZE depth=(RM_PSA_LUT_SIZE/PORT_VEC_SIZE) num_write_outstanding=1 max_write_burst_length=2
	...
	static model_psa_t psa_lut_buff[PSA_LUT_BUFF_N][PSA_LUT_BUFF_DIM_0][PSA_LUT_BUFF_DIM_1];
	#pragma HLS array_partition variable=psa_lut_buff dim=1 type=complete
	// Add this pragma if using the processor_old.cpp with unoptimized PSA
	// #pragma HLS array_partition variable=psa_lut_buff dim=2 type=cyclic factor=(LERP_PSA_N/PSA_LUT_BUFF_DIM_1)
	#pragma HLS array_partition variable=psa_lut_buff dim=3 type=complete
	...
	// #pragma HLS allocation function instances=ld_processor_psa_affine_sub_task limit=1
	...
	#pragma HLS bind_storage variable=psa_lut_buff type=RAM_2P impl=bram
